...
def test_gvchk():
    ...
    assert gv_rx_chk(
        bytes.fromhex("AA0D0000000000000000000000000000000000A7")
    ) == bytes.fromhex("AA0D0000000000000000000000000000000000")
    ...
    assert gv_tx_chk(bytes.fromhex("AA0D")) == bytes.fromhex(
        "AA0D0000000000000000000000000000000000A7"
    )
...
def test_h5179():
    ...
    assert gh5179.advertisement(advertisement) == {
        "bat": 88,
        "humid": 55.9,
        "temp": 17.7,
    }
...
def test_h5174():
    ...
    advertisement = AdvertisementData(
        local_name="GVH5174_6BE0",
        manufacturer_data={
            1: b"\x01\x01\x02\xf7\xd6d",
    ...
    assert gh5174.advertisement(advertisement) == {
        "bat": 100,
        "humid": 51.8,
        "temp": 19.4,
    }
